...
fn efi_main(_handle: Handle, mut system_table: SystemTable<Boot>) -> Status {
    ...
    // For ELF files, entry point is a physical address where kernel should execute, not an offset within the loaded file buffer
    ...
    system_table.stdout().output_string(cstr16!("\r\n")).unwrap_or(());
    system_table.stdout().output_string(cstr16!("╔══════════════════════════════════════════════════════════════════════╗\r\n")).unwrap_or(());
    system_table.stdout().output_string(cstr16!("║                     BOOT SEQUENCE COMPLETE                          ║\r\n")).unwrap_or(());
    system_table.stdout().output_string(cstr16!("║              Transferring Control to NØNOS Kernel                   ║\r\n")).unwrap_or(());
    system_table.stdout().output_string(cstr16!("╚══════════════════════════════════════════════════════════════════════╝\r\n")).unwrap_or(());
    system_table.stdout().output_string(cstr16!("\r\n")).unwrap_or(());
    ...
    // Transfer control to kernel - no more UEFI services available beyond this point
