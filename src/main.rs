...
use nonos_boot::config::{apply_configuration, display_configuration, load_bootloader_config};
...
use nonos_boot::log::logger::{log_critical, log_debug, log_error, log_info, log_warn};
use nonos_boot::multiboot::MultiBootManager;
use nonos_boot::network::{display_network_boot_menu, initialize_network_boot, NetworkBootOption};
...
fn efi_main(_handle: Handle, mut system_table: SystemTable<Boot>) -> Status {
    ...
    system_table
        .stdout()
        .output_string(cstr16!(
            "║              >>> YOUR CUSTOM OS IS RUNNING IN QEMU! <<<             ║\r\n"
        ))
        .unwrap_or(());
    system_table
        .stdout()
    ...
    log_info(
        "boot",
        "NØNOS Capsule Bootloader Activated - Advanced Professional Version",
    );
    ...
    system_table
        .stdout()
        .output_string(cstr16!(
            "║  Features: ACPI • Security • Networking • TPM • Config • Crypto     ║\r\n"
        ))
        .unwrap_or(());
    system_table
        .stdout()
    ...
    system_table
        .stdout()
        .output_string(cstr16!("Phase 0: Configuration Loading\r\n"))
        .unwrap_or(());
    ...
    system_table
        .stdout()
        .output_string(cstr16!("Phase 1: Security Initialization\r\n"))
        .unwrap_or(());
    let security_context = initialize_security_subsystem(&mut system_table);

    // Assess and display security posture
    let security_score =
        nonos_boot::security::assess_security_posture(&security_context, &mut system_table);

    // Phase 2: Hardware Discovery & ACPI
    system_table
        .stdout()
        .output_string(cstr16!("Phase 2: Hardware Discovery\r\n"))
        .unwrap_or(());
    ...
    system_table
        .stdout()
        .output_string(cstr16!("Phase 3: Network Subsystem\r\n"))
        .unwrap_or(());
    let network_context = initialize_network_boot(&mut system_table);

    // Perform network diagnostics and security assessment
    let network_diagnostics_passed =
        nonos_boot::network::perform_network_diagnostics(&mut system_table, &network_context);
    let network_security_score =
        nonos_boot::network::assess_network_security(&mut system_table, &network_context);

    // Apply configuration to all subsystems
    system_table
        .stdout()
        .output_string(cstr16!("Phase 3.5: Configuration Application\r\n"))
        .unwrap_or(());
    ...
    system_table
        .stdout()
        .output_string(cstr16!("Phase 4: Graphics & Memory\r\n"))
        .unwrap_or(());
    ...
    system_table
        .stdout()
        .output_string(cstr16!("Phase 5: Multi-Boot & Boot Source Selection\r\n"))
        .unwrap_or(());
    ...
    let selected_boot_entry =
        multiboot_manager.display_boot_menu(&mut system_table, &bootloader_config);
    ...
    system_table
        .stdout()
        .output_string(cstr16!("Phase 6: Kernel Loading\r\n"))
        .unwrap_or(());
    ...
    if let Some(_entry) = multiboot_manager.get_entry_info(entry_id) {
        ...
        system_table
            .stdout()
            .output_string(cstr16!("   [INFO] Multi-boot entry selected\r\n"))
            .unwrap_or(());
    ...
    let kernel_capsule = match boot_option {
        NetworkBootOption::Pxe => {
            system_table
                .stdout()
                .output_string(cstr16!("   [INFO] Attempting PXE kernel load...\r\n"))
                .unwrap_or(());
            match nonos_boot::network::load_kernel_via_pxe(&mut system_table, &network_context) {
                Ok(_kernel_data) => {
                    ...
                    system_table
                        .stdout()
                        .output_string(cstr16!("   [INFO] Falling back to local boot...\r\n"))
                        .unwrap_or(());
                ...
                Err(_) => {
                    system_table
                        .stdout()
                        .output_string(cstr16!("   [WARN] PXE boot failed, trying local...\r\n"))
                        .unwrap_or(());
                    load_kernel_capsule(&mut system_table)
                }
            }
        ...
        NetworkBootOption::Http => {
            system_table
                .stdout()
                .output_string(cstr16!("   [INFO] Attempting HTTP kernel load...\r\n"))
                .unwrap_or(());
            match nonos_boot::network::load_kernel_via_http(
                ...
                Err(_) => {
                    system_table
                        .stdout()
                        .output_string(cstr16!("   [WARN] HTTP boot failed, trying local...\r\n"))
                        .unwrap_or(());
                    load_kernel_capsule(&mut system_table)
                }
            }
        ...
        NetworkBootOption::Local => {
            system_table
                .stdout()
                .output_string(cstr16!(
                    "   [INFO] Loading kernel from local storage...\r\n"
                ))
                .unwrap_or(());
            load_kernel_capsule(&mut system_table)
        }
    ...
    let kernel_capsule = match kernel_capsule {
        Ok(kc) => {
            ...
            let advanced_verification =
                nonos_boot::security::verify_kernel_signature_advanced(kernel_data);
            if advanced_verification {
                system_table
                    .stdout()
                    .output_string(cstr16!(
                        "   [SUCCESS] Advanced signature verification passed\r\n"
                    ))
                    .unwrap_or(());
                log_info(
                    "crypto",
                ...
                system_table
                    .stdout()
                    .output_string(cstr16!(
                        "   [WARN] Advanced verification failed, using standard verification\r\n"
                    ))
                    .unwrap_or(());
                log_warn(
                    "crypto",
                    "Advanced verification failed, continuing with standard verification",
                );
            }
            ...
            if security_context.measured_boot_active {
                ...
                if nonos_boot::security::extend_pcr_measurement(
                    &mut system_table,
                    4,
                    measurement_data,
                ) {
                    system_table
                        .stdout()
                        .output_string(cstr16!("   [SUCCESS] Kernel measured in TPM\r\n"))
                        .unwrap_or(());
                    ...
                    if advanced_verification {
                        ...
                        if nonos_boot::security::extend_pcr_measurement(
                            ...
                            system_table
                                .stdout()
                                .output_string(cstr16!(
                                    "   [SUCCESS] Advanced crypto event measured\r\n"
                                ))
                                .unwrap_or(());
                            log_info(
                                "tpm",
                                "Advanced crypto verification event measured in PCR 5",
                            );
                        }
                    }
                }
        ...
        Err(e) => {
            system_table
                .stdout()
                .output_string(cstr16!("   [ERROR] All kernel load methods failed\r\n"))
                .unwrap_or(());
    ...
    if kernel_capsule.entry_point < 0x100000 || kernel_capsule.entry_point >= 0x10000000 {
        system_table
            .stdout()
            .output_string(cstr16!("   [ERROR] Entry point validation failed\r\n"))
            .unwrap_or(());
        log_error("security", "Entry point outside valid kernel address range");
        fatal_reset(
            &mut system_table,
            "Entry point outside valid kernel address range",
        );
    }

    system_table
        .stdout()
        .output_string(cstr16!("   [SUCCESS] Entry point validated\r\n"))
        .unwrap_or(());

    // Phase 7: Final Handoff Preparation
    system_table
        .stdout()
        .output_string(cstr16!("Phase 7: Kernel Handoff\r\n"))
        .unwrap_or(());
    ...
    system_table
        .stdout()
        .output_string(cstr16!("   [INFO] Security subsystem ready\r\n"))
        .unwrap_or(());
    system_table
        .stdout()
        .output_string(cstr16!("   [INFO] Hardware discovery complete\r\n"))
        .unwrap_or(());

    if network_diagnostics_passed {
        system_table
            .stdout()
            .output_string(cstr16!("   [INFO] Network subsystem fully operational\r\n"))
            .unwrap_or(());
    } else {
        system_table
            .stdout()
            .output_string(cstr16!("   [WARN] Network subsystem has limitations\r\n"))
            .unwrap_or(());
    }
    ...
    system_table
        .stdout()
        .output_string(cstr16!(
            "   [INFO] Advanced cryptographic verification active\r\n"
        ))
        .unwrap_or(());
    ...
    if config_applied {
        system_table
            .stdout()
            .output_string(cstr16!(
                "   [INFO] Configuration policies fully enforced\r\n"
            ))
            .unwrap_or(());
    } else {
        system_table
            .stdout()
            .output_string(cstr16!(
                "   [WARN] Configuration application had some issues\r\n"
            ))
            .unwrap_or(());
    }
    ...
    system_table
        .stdout()
        .output_string(cstr16!("   [INFO] Multi-boot system operational\r\n"))
        .unwrap_or(());
    system_table
        .stdout()
        .output_string(cstr16!("   [INFO] Kernel handoff data prepared\r\n"))
        .unwrap_or(());
    ...
    if overall_security >= 120 {
        system_table
            .stdout()
            .output_string(cstr16!(
                "   [SUCCESS] Overall system security: EXCELLENT\r\n"
            ))
            .unwrap_or(());
    } else if overall_security >= 80 {
        system_table
            .stdout()
            .output_string(cstr16!("   [INFO] Overall system security: GOOD\r\n"))
            .unwrap_or(());
    } else {
        system_table
            .stdout()
            .output_string(cstr16!("   [WARN] Overall system security: MODERATE\r\n"))
            .unwrap_or(());
    }
    ...
    if bootloader_config.verbose_logging {
        system_table
            .stdout()
            .output_string(cstr16!("   [INFO] Verbose logging mode active\r\n"))
            .unwrap_or(());
    }
    ...
    if bootloader_config.diagnostic_output {
        system_table
            .stdout()
            .output_string(cstr16!("   [INFO] Diagnostic output mode active\r\n"))
            .unwrap_or(());
        if testing_passed {
            system_table
                .stdout()
            ...
            system_table
                .stdout()
                .output_string(cstr16!("   [WARN] Some comprehensive tests failed\r\n"))
                .unwrap_or(());
        }
    } else if testing_passed {
        system_table
            .stdout()
            .output_string(cstr16!("   [INFO] Quick health check passed\r\n"))
            .unwrap_or(());
    } else {
        system_table
            .stdout()
            .output_string(cstr16!("   [WARN] Health check had issues\r\n"))
            .unwrap_or(());
    }
...
fn initialize_graphics(system_table: &mut SystemTable<Boot>) {
    ...
    if graphics_initialized > 0 {
        system_table
            .stdout()
            .output_string(cstr16!("   [SUCCESS] Graphics Output Protocol found\r\n"))
            .unwrap_or(());
    } else {
        system_table
            .stdout()
            .output_string(cstr16!(
                "   [INFO] No graphics support, using text mode\r\n"
            ))
            .unwrap_or(());
...
fn setup_memory_management(system_table: &mut SystemTable<Boot>) {
    system_table
        .stdout()
        .output_string(cstr16!("   [INFO] Memory map analysis...\r\n"))
        .unwrap_or(());
    ...
    if let Ok(buffer_ptr) = bs.allocate_pages(
        ...
        system_table
            .stdout()
            .output_string(cstr16!("   [WARN] Memory map unavailable\r\n"))
            .unwrap_or(());
...
fn discover_hardware(system_table: &mut SystemTable<Boot>) {
    system_table
        .stdout()
        .output_string(cstr16!("   [INFO] Scanning for hardware components...\r\n"))
        .unwrap_or(());
    ...
    if storage_found {
        system_table
            .stdout()
            .output_string(cstr16!("   [INFO] Block storage devices found\r\n"))
            .unwrap_or(());
    }
    if network_found {
        system_table
            .stdout()
            .output_string(cstr16!("   [INFO] Network interfaces found\r\n"))
            .unwrap_or(());
    }
    if filesystem_found {
        system_table
            .stdout()
            .output_string(cstr16!("   [INFO] File systems found\r\n"))
            .unwrap_or(());
    }

    system_table
        .stdout()
        .output_string(cstr16!("   [SUCCESS] Hardware discovery complete\r\n"))
        .unwrap_or(());
