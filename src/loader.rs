//! NØNOS Capsule Loader (UEFI FS → verified capsule → handoff build)
//
// Responsibilities:
...
pub fn load_kernel_capsule(st: &mut SystemTable<Boot>) -> Result<KernelCapsule, &'static str> {
    ...
    let elf_file = ElfFile::new(&capsule_slice[..bytes_read])
        .map_err(|_| "[x] Invalid ELF file format")?;
        ...
        if let Ok(memory_map) = bs.memory_map(buffer) {
            ...
            for descriptor in memory_map.entries() {
                ...
                if !found_usable && 
                   ...
                   descriptor.phys_start >= 0x100000 { // Above 1MB
