//! Configuration System for NØNOS Bootloader
//!
...
pub fn load_bootloader_config(system_table: &mut SystemTable<Boot>) -> BootloaderConfig {
    ...
    system_table.stdout().output_string(cstr16!("=== Loading Bootloader Configuration ===\r\n")).unwrap_or(());
    ...
    if let Some(policy) = security_policy {
        ...
        system_table.stdout().output_string(cstr16!("   [SUCCESS] Security policy loaded from NVRAM\r\n")).unwrap_or(());
        log_info("config", "Security policy loaded from NVRAM");
    } else {
        system_table.stdout().output_string(cstr16!("   [INFO] Using default security policy\r\n")).unwrap_or(());
    ...
    if let Some(policy) = network_policy {
        ...
        system_table.stdout().output_string(cstr16!("   [SUCCESS] Network policy loaded from NVRAM\r\n")).unwrap_or(());
        log_info("config", "Network policy loaded from NVRAM");
    } else {
        system_table.stdout().output_string(cstr16!("   [INFO] Using default network policy\r\n")).unwrap_or(());
    ...
    if let Some(timeout) = boot_timeout {
        ...
        system_table.stdout().output_string(cstr16!("   [SUCCESS] Boot timeout loaded from NVRAM\r\n")).unwrap_or(());
    ...
    if let Some(mode) = graphics_mode {
        ...
        system_table.stdout().output_string(cstr16!("   [SUCCESS] Graphics mode loaded from NVRAM\r\n")).unwrap_or(());
...
pub fn apply_configuration(
    ...
    system_table.stdout().output_string(cstr16!("=== Applying Configuration ===\r\n")).unwrap_or(());
    
    let mut application_successful = true;
    
    // Apply security policy
    if !apply_security_policy(config, system_table, security) {
        system_table.stdout().output_string(cstr16!("   [ERROR] Failed to apply security policy\r\n")).unwrap_or(());
        log_error("config", "Failed to apply security policy");
        application_successful = false;
    } else {
        system_table.stdout().output_string(cstr16!("   [SUCCESS] Security policy applied\r\n")).unwrap_or(());
        log_info("config", "Security policy applied successfully");
    }
    
    // Apply network policy
    if !apply_network_policy(config, system_table, network) {
        system_table.stdout().output_string(cstr16!("   [WARN] Network policy application had issues\r\n")).unwrap_or(());
        log_warn("config", "Network policy application had issues");
    } else {
        system_table.stdout().output_string(cstr16!("   [SUCCESS] Network policy applied\r\n")).unwrap_or(());
        log_info("config", "Network policy applied successfully");
    }
    
    // Apply hardware optimizations
    if config.cpu_optimizations && hardware.cpu_count > 1 {
        system_table.stdout().output_string(cstr16!("   [INFO] CPU optimizations enabled for multi-core system\r\n")).unwrap_or(());
    ...
    match config.memory_management_mode {
        MemoryManagementMode::Secure => {
            system_table.stdout().output_string(cstr16!("   [INFO] Secure memory management mode active\r\n")).unwrap_or(());
            log_info("config", "Secure memory management mode applied");
        }
        MemoryManagementMode::Efficient => {
            system_table.stdout().output_string(cstr16!("   [INFO] Efficient memory management mode active\r\n")).unwrap_or(());
            log_info("config", "Efficient memory management mode applied");
        }
        MemoryManagementMode::Legacy => {
            system_table.stdout().output_string(cstr16!("   [INFO] Legacy memory management mode active\r\n")).unwrap_or(());
...
fn apply_security_policy(
    ...
    match config.security_policy {
        SecurityPolicy::Maximum => {
            ...
            if !security.secure_boot_enabled || !security.tpm_available || !security.platform_key_verified {
                log_error("config", "Maximum security policy requirements not met");
                return false;
            }
            system_table.stdout().output_string(cstr16!("   [INFO] Maximum security policy enforced\r\n")).unwrap_or(());
        }
        SecurityPolicy::Standard => {
            ...
            system_table.stdout().output_string(cstr16!("   [INFO] Standard security policy enforced\r\n")).unwrap_or(());
        }
        SecurityPolicy::Relaxed => {
            // Relaxed security - minimal requirements
            system_table.stdout().output_string(cstr16!("   [INFO] Relaxed security policy enforced\r\n")).unwrap_or(());
        }
        SecurityPolicy::Custom => {
            // Custom security policy - user-defined
            system_table.stdout().output_string(cstr16!("   [INFO] Custom security policy enforced\r\n")).unwrap_or(());
        }
    }
...
fn apply_network_policy(
    ...
    match config.network_policy {
        NetworkPolicy::Disabled => {
            system_table.stdout().output_string(cstr16!("   [INFO] Network boot disabled by policy\r\n")).unwrap_or(());
            log_info("config", "Network boot disabled by policy");
        }
        NetworkPolicy::Secured => {
            if !network.http_client_available {
                system_table.stdout().output_string(cstr16!("   [WARN] Secured network policy requires HTTPS support\r\n")).unwrap_or(());
                log_warn("config", "Secured network policy requirements not fully met");
                return false;
            }
            system_table.stdout().output_string(cstr16!("   [INFO] Secured network policy enforced\r\n")).unwrap_or(());
        }
        NetworkPolicy::Standard => {
            system_table.stdout().output_string(cstr16!("   [INFO] Standard network policy enforced\r\n")).unwrap_or(());
        }
        NetworkPolicy::Unrestricted => {
            system_table.stdout().output_string(cstr16!("   [INFO] Unrestricted network policy enforced\r\n")).unwrap_or(());
        }
    }
...
pub fn display_configuration(config: &BootloaderConfig, system_table: &mut SystemTable<Boot>) {
    system_table.stdout().output_string(cstr16!("=== Current Configuration ===\r\n")).unwrap_or(());
    
    // Security configuration
    match config.security_policy {
        SecurityPolicy::Maximum => {
            system_table.stdout().output_string(cstr16!("Security Policy:   MAXIMUM\r\n")).unwrap_or(());
        }
        SecurityPolicy::Standard => {
            system_table.stdout().output_string(cstr16!("Security Policy:   STANDARD\r\n")).unwrap_or(());
        }
        SecurityPolicy::Relaxed => {
            system_table.stdout().output_string(cstr16!("Security Policy:   RELAXED\r\n")).unwrap_or(());
        }
        SecurityPolicy::Custom => {
            system_table.stdout().output_string(cstr16!("Security Policy:   CUSTOM\r\n")).unwrap_or(());
        }
    }
    ...
    match config.network_policy {
        NetworkPolicy::Disabled => {
            system_table.stdout().output_string(cstr16!("Network Policy:    DISABLED\r\n")).unwrap_or(());
        }
        NetworkPolicy::Secured => {
            system_table.stdout().output_string(cstr16!("Network Policy:    SECURED\r\n")).unwrap_or(());
        }
        NetworkPolicy::Standard => {
            system_table.stdout().output_string(cstr16!("Network Policy:    STANDARD\r\n")).unwrap_or(());
        }
        NetworkPolicy::Unrestricted => {
            system_table.stdout().output_string(cstr16!("Network Policy:    UNRESTRICTED\r\n")).unwrap_or(());
        }
    }
    ...
    match config.preferred_boot_method {
        PreferredBootMethod::Local => {
            system_table.stdout().output_string(cstr16!("Boot Method:       LOCAL PREFERRED\r\n")).unwrap_or(());
        }
        PreferredBootMethod::Network => {
            system_table.stdout().output_string(cstr16!("Boot Method:       NETWORK PREFERRED\r\n")).unwrap_or(());
        }
        PreferredBootMethod::Intelligent => {
            system_table.stdout().output_string(cstr16!("Boot Method:       INTELLIGENT SELECTION\r\n")).unwrap_or(());
        }
    }
    
    // Other settings
    if config.verbose_logging {
        system_table.stdout().output_string(cstr16!("Verbose Logging:   ENABLED\r\n")).unwrap_or(());
    } else {
        system_table.stdout().output_string(cstr16!("Verbose Logging:   DISABLED\r\n")).unwrap_or(());
    }
...
pub fn save_configuration(config: &BootloaderConfig, system_table: &mut SystemTable<Boot>) -> bool {
    ...
    if policy_saved {
        system_table.stdout().output_string(cstr16!("   [SUCCESS] Security policy saved\r\n")).unwrap_or(());
        log_info("config", "Security policy saved to NVRAM");
    } else {
        system_table.stdout().output_string(cstr16!("   [ERROR] Failed to save security policy\r\n")).unwrap_or(());
    ...
    if network_policy_saved {
        system_table.stdout().output_string(cstr16!("   [SUCCESS] Network policy saved\r\n")).unwrap_or(());
        log_info("config", "Network policy saved to NVRAM");
    } else {
        system_table.stdout().output_string(cstr16!("   [ERROR] Failed to save network policy\r\n")).unwrap_or(());
